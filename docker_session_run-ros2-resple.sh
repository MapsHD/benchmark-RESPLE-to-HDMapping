...
echo "=== Resple ROS2 rosbag pipeline ==="
...
docker run -it --rm \
  ...
  /bin/bash -c "
    source /opt/ros/humble/setup.bash
    source /ros2_ws/install/setup.bash
    ros2 run resple-to-hdmapping listener \
      \"$BAG_OUTPUT_CONTAINER/$RECORDED_BAG_NAME\" \
      \"$BAG_OUTPUT_CONTAINER/$HDMAPPING_OUT_NAME-resple\"
  "
